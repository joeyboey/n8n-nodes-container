...
export class Kubernetes implements INodeType {
	...
	async execute(
		...
		for (let idx = 0; idx < this.getInputData().length; idx++) {
			...
			let data: IDataObject | undefined = undefined;
			...
			if (operation === 'run') {
				...
				const namespace = (this.getNodeParameter('namespace', idx) as string) ?? 'default';
				if (!Array.isArray(command)) {
					throw new NodeOperationError(this.getNode(), 'Command must be an array!');
				}

				data = {
					stdout: await k8s.runPodAndGetOutput(image, command, undefined, namespace),
				};
			}
