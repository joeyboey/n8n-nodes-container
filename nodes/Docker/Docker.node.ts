 ...
 */
function buildVolumeConfig(volumeMounts: VolumeMountConfig): {
...
export class Docker implements INodeType {
	description: INodeTypeDescription = {
		...
		properties: [
			...
			},
			{
						...
						values: [
							{
								...
								description: 'Path on the host machine or Docker volume name',
								required: true,
							},
							{
				...
				description: 'Mount host directories or Docker volumes into the container',
			},
			{
	...
	async execute(
		...
		for (let idx = 0; idx < this.getInputData().length; idx++) {
			const credentials = await this.getCredentials('dockerCredentialsApi', idx);
			...
			if (operation === 'run') {
				...
				const command = JSON.parse(this.getNodeParameter('command', idx) as string);
				if (!Array.isArray(command)) {
					throw new NodeOperationError(this.getNode(), 'Command must be an array!');
				}

				// Get volume mount configuration
				const volumeMounts = this.getNodeParameter('volumeMounts', idx, {}) as VolumeMountConfig;
				...
				const workingDir = this.getNodeParameter('workingDir', idx, '') as string;
